window.addEventListener('DOMContentLoaded', function () {
  ...
  function initFetch() {
    fetch('https://freegeoip.app/json/')
      ...
      .then(function (country) {
        qS('#country-name').innerHTML = country.country_name;

        fetch(
          'https://covid19.mathdro.id/api/countries/' + country.country_code
        )
          ...
          .catch(function () {
            qS('.page-loading .txt-loading').innerHTML =
              'Hmmm! An error occurred';
          });
      })
      .catch(function () {
    ...
    fetch('https://covid19.mathdro.id/api/countries/')
      ...
      .then(function (data) {
        ...
        for (var i in countries) {
          dropdownItem = '<a href="#!" class="dropdown-item" data-id="' +
            countries[i].iso3 +
            '"> ' +
            countries[i].name +
            '</a>';
  ...
  function initDropdownItemCountry(self) {
    ...
    fetch('https://covid19.mathdro.id/api/countries/' + iso3)
      ...
      .then(function (data) {
        ...
        qS(
          '#dropdown-country .dropdown-trigger button .txt-select'
  ...
  function updateCovidSummary(data) {
    qS('#cov19-summary .confirmed').innerHTML = numberWithDots(
      data.confirmed.value
    );
    qS('#cov19-summary .recovered').innerHTML = numberWithDots(
      data.recovered.value
    );
    qS('#cov19-summary .deaths').innerHTML = numberWithDots(data.deaths.value);
  }
  ...
  function createNotification(type, message) {
    ...
    setTimeout(function () {
      var isNotif = mainContent.childNodes[0].classList.contains(
        'notification'
      );
      if (isNotif) {
